...
- Per-channel queues (`src/queue.js`): prompts sharing a `channelId` are FIFO; different channels run concurrently. One global maintenance gate protects login, shell and upgrades. Text messages sent while their channel is busy are merged into one turn (see "Execution queues").
...
src/
  ...
  prompts.js          # Shared system prompt builder (agent-agnostic — no per-agent sections) + the scheduling reference shipped as a file (getSchedulingDoc); injects the mode home's optional `SOUL.md` (persona) into both the agent prompt and the GPT-Live instructions
...
- **Delegation** (`src/voice.js`): the call has no tools and delegates every task (instructions: `prompts.js::getLiveInstructions`, which embeds the mode home's `SOUL.md` so the voice and the agent share one persona; read at call creation, so an edit applies to the next call). `delegation.created` carries the full request text → `executePrompt(agent, mode, text)` through the channel FIFO at the `high` tier, without blocking the conversation. Progress goes back as silent `commentary` context (throttled) and to the chat as the text channels' live activity line (`startProgressReporter`, without the typing indicator), the result as `speakable` context, which the call paraphrases; request and full reply are posted to the chat. A delegation made while an earlier one is still pending is prefixed with a note that it was worded without that result and may correct it — the backend, which resumes the session at the head of the queue, has the result. There is no "delegation done" event on this wire. The session is keyed by the voice channel's own `channelId` (text-in-voice shares it), so voice and chat share one conversation.
...
- **Lifecycle**: the call dies with the session and vice versa (`session.closed`, sideband or WebRTC loss → leave with a chat notice; sessions expire ~2 h after start). Each join resets the channel's agent session (implicit `/new`, skipped while a task still runs there; it deletes the channel's non-isolated jobs, so the voice prompt requires isolated ones); `/new`, `/admin`, `/sandbox` restart the call, so the call and the agent session always start fresh together. Delegations still running when the call ends report to the chat only. Agent switches are locked while the assistant is active there — the shared sessionId (Claude UUID vs Codex thread id) must stay coherent with the executor's context guard. `scheduler.reloadJobs()` after each delegation (like index.js). Auto-leave after 15 min without a user turn (`VOICE_IDLE_TIMEOUT_MS`), suspended while delegations are pending.
...
Interactive prompts, voice turns and scheduled jobs go through `src/queue.js::runQueued(channelId, ...)`. A channel or thread stays strictly FIFO, including its job runs, while distinct IDs may execute without a global concurrency limit. `src/index.js` merges the text messages sent while a channel is busy into one batch (joined with blank lines, marked with a ⏳ reaction); `executePrompt` takes the prompt as a function and closes the batch when it reaches the head of the queue. Discord cannot reorder messages and a bot cannot delete a user's DM messages, so the batch is echoed as a quote (📨) right before its answer. A mode/agent change starts a new batch. Rare operations (`/login`, `!shell`, `/upgrade`) use `runMaintenance`: they are refused while any execution is pending, then briefly stop new queue work while they run. The single sandbox container accepts concurrent `docker exec` processes, so sandbox channels share its 1 CPU, home and filesystem. SQLite arbitrates concurrent jobs writes.
...
- `ADMIN_USER_HOME/.claudiscord/sessions.json` shape:
  ```json
  { "channels": { "<channelId>": { "mode": "admin"|"sandbox", "agent": "claude"|"codex", "sessionId": "<uuid>", "context": null|<tokens>, "lastName": "...", "depotPath": null|"/abs/path", "diffGistId": null|"<gistId>" } } }
  ```
...
- `context` backs `/status`'s conversation line: the LAST assistant event's input tokens, cache included — `result.usage` sums every model call of the turn, so a three-tool run reported 90k against a 22k conversation. A snapshot, replaced each turn. Recorded on failed and cancelled turns too, which grew the conversation all the same. Codex sets none: `turn.completed.usage` is a turn total and Codex exposes no per-request input, so nothing there answers "how full is this conversation".
