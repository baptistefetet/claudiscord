...
- Voice turns run the channel's agent/mode/model. Agent switches (`/codex`, or `/opus`/`/sonnet` on a codex channel) are locked while the assistant is active there — the shared sessionId (Claude UUID vs Codex thread id) must stay coherent with the executor's context guard. **The bot joins silently** — the mode goes to the channel's chat, not the speakers: a spoken greeting on every connect grates once autojoin makes it routine, and it was clipped anyway (autojoin fires while the user's own client is still finishing its voice handshake, and the cached clip has no TTS round-trip left to mask it). `connectAndStart` instead settles the player straight to Paused: the idle bed is silent (`BED_IDLE = 0`) but a live player keeps transmitting and shows the bot as permanently speaking. It waits for `AudioPlayerStatus.Playing` first — `pause()` is a no-op unless the player is already Playing, and `play()` starts in Buffering. Auto-leave after 15 min without a turn (`VOICE_IDLE_TIMEOUT_MS`); the idle timer is suspended during a turn.
...
- **Opt-in per channel = the allowlist.** Without it, the bot would sit in any call the user joins and turn every sentence they say to a human into a prompt (`onSpeakingStart` only reacts to `AUTHORIZED_USER_ID`, which is exactly the problem). No global env var: the flag lives where the feature lives, needs no restart, and `/status` shows it when on.
- **Policy vs session are orthogonal**: `/autojoin` off does NOT disconnect a bot already connected (`/voice` does that); `/voice` off does NOT clear the policy — it suppresses it for the current stay only (`suppressed` Set, in-memory, cleared when the user leaves the channel, so autojoin re-arms on the next connect). Only the explicit `/voice` kick passes `leaveVoice({ suppressAutojoin: true })` — the idle timeout, a disconnect and shutdown don't need it (autojoin is only re-triggered by a `voiceStateUpdate`, which none of them produce). Suppression is recorded only when the policy is actually on, and `/autojoin` on calls `clearAutojoinSuppression()` — an explicit enable is an explicit re-arm, otherwise a `/voice` kick earlier in the same stay would silently swallow the immediate join.
- **Known deviation**: `suppressed` is in-memory, `autojoin` is persisted, so a `/restart` while the user sits in a channel they had kicked the bot from re-joins on the boot scan. Accepted: a restart drops all live session state by design and the policy is what survives.
- **Move semantics**: user moves to a channel with autojoin → the bot follows; to a channel without → the bot just leaves. "Following" is `leaveVoice()` + `joinVoice()`, never a connection move: the session is keyed by the voice channel's own id, so the destination brings its own mode/agent/model/sessionId and re-announces its mode. The conversation does NOT follow the user.
- **`voiceStateUpdate` gotchas** (`src/voice.js::handleVoiceStateUpdate`): the event also fires on mute, deafen, streaming and camera (`oldState.channelId === newState.channelId` → early return, otherwise every mute re-runs the join logic), and for every member *including the bot itself*, whose own join/leave — plus the leave+join burst of a follow — would re-enter the logic. Both filtered up front.
- **Join race**: `active` cannot serialize joins on its own — it is assigned at the very END of `joinVoice`, after `entersState` has awaited readiness for up to 15 s. Inside that window a second caller sees `active === null` and starts a parallel session; since `@discordjs/voice` reuses the connection per guild, both would attach a speaking handler to the same receiver (duplicate turns) and the overwritten session would leak past `leaveVoice`'s reach. Hence the `joining` guard lives **inside `joinVoice`**, covering every path (`/voice`, autojoin, boot scan), not just autojoin.
- **Convergence**: every autojoin attempt is followed by `reconcileAutojoin()`, unconditionally — the join window (plus the chat post) is wide enough for the user to have left or moved on, and their `voiceStateUpdate` was either dropped by the `joining` guard or found no session to tear down, so nothing else would fix it. It also recovers a *failed* join whose destination event was swallowed the same way. The `seen` Set threaded through the chain guarantees termination: a channel that always fails to join (missing Connect permission) is attempted once per chain, not forever. Each new event starts a fresh chain.
- **Boot scan**: the gateway only delivers `voiceStateUpdate` on *changes*, so a user already sitting in an autojoin channel at boot (typical after `/restart`) never triggers one. `scanAutojoinOnBoot()` converges once on `ClientReady`, after `purgeInvalidChannels`, scoped to opted-in channels.
- **Silent join**: there is deliberately no spoken greeting (see the join note in the section above) — autojoin makes one on every connect, which both grates and clips.
- **Layering**: `index.js` only forwards the Discord event (sync listener → `.catch`, discord.js does not await handlers); all the policy lives in `voice.js`. Same split as the slash-command path.
