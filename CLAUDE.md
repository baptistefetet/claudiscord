...
- **Mixer** (`src/mixer.js`): ONE permanent Readable (s16le 48 kHz stereo) plays for the whole session; a synthesized detuned-sine "thinking" bed (gain 0.18) is ducked to 0.06 under speech, gains smoothed per 20 ms frame. The player is paused between turns so the speaking indicator turns off while idle (the lib's own 5 s UDP keepalive keeps the session alive). A player `Idle` means the pipeline broke → the mixer resource is rebuilt.
