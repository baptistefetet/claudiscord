...
- **Pipeline** (`src/voice.js`): `receiver.subscribe(AfterSilence 900ms)` → prism opus decode → ffmpeg → 16 kHz mono WAV → Groq Whisper (`stt.js::transcribeAudio`) → gate (min 300 ms, French Whisper hallucination patterns) → `executePrompt(agent, mode, text)` → OpenAI TTS (`tts.js`) → ffmpeg mp3→PCM → mixer playback. The session is keyed by the voice channel's own `channelId` (text-in-voice shares it), so voice and chat share one conversation and `/admin`, `/sandbox`, `/status` typed in the chat apply.
