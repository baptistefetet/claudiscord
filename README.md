...
- **Fixed models** — your prompts use the agent's high model (Claude `opus`, Codex `sol`), scheduled jobs its medium one (`sonnet` / `luna`), reasoning effort `xhigh` everywhere. Nothing to pick
...
The container runs the host's own Claude and Codex binaries through read-only bind-mounts, so you install and update each agent once, on the host, and the sandbox follows. Configuration stays separate — credentials, skills, `AGENTS.md` and the optional `SOUL.md` persona (injected into the agent prompt and the voice call) are read from each environment's home, so an admin channel and a sandbox channel remain distinct agents.
